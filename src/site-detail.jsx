...
import {cachedFetch, setAppTitle, setSiteDetailList, setSiteList,setAppIcon} from "./actions";
...
class SiteDetail extends React.Component {
    ...
    norm(text){
        var arr=text.split ('').map (function (c) { return c.charCodeAt (0).toString(16); });
        return arr.join("-");
    }
                        ...
                        return (
                            <ListItem
                                ...
                                onClick={() => {
                                    ...
                                    this.props.history.push('/site-category/' + state.lastDetail + '/'+this.norm(item.text)+'/' + this.norm(item.link))
                                }}
